...
function renderDashboard(){
  ...
  stats.innerHTML=`
    <div class="dash-stat" onclick="drawerNavigate('vehicles')"><div class="dash-stat-icon">🚗</div><div class="dash-stat-value">${vehicles.length}</div><div class="dash-stat-label">Toplam Araç</div></div>
    <div class="dash-stat" onclick="drawerNavigate('vehicles')"><div class="dash-stat-icon">🟢</div><div class="dash-stat-value">${active}</div><div class="dash-stat-label">Aktif Araç</div></div>
    <div class="dash-stat" onclick="drawerNavigate('inactive')"><div class="dash-stat-icon">⚪</div><div class="dash-stat-value">${inactive}</div><div class="dash-stat-label">Pasif Araç</div></div>`;
  ...
  tasks.innerHTML=`<div class="task-summary">${urgentVehicles} araçla ilgilenmen gerekiyor</div>`+shown.map(it=>{
    const dtext=it.days<0?`${Math.abs(it.days)} gün gecikti`:it.days===0?'Bugün':`${it.days} gün`;
    const color=it.level==='red'?'var(--red)':'var(--amber)';
    return `<div class="task-row" onclick="openModal('${it.vehicle.id}','${it.key}')"><span class="task-dot" style="background:${color}"></span><div class="task-main"><div class="task-plate">${escapeHtml((it.vehicle.plate||'—').toUpperCase())}</div><div class="task-copy">${escapeHtml(it.label)} · ${escapeHtml([it.vehicle.brand,it.vehicle.model].filter(Boolean).join(' '))}</div></div><div class="task-days" style="color:${color}">${dtext}</div></div>`;
  }).join('')+(taskItems.length>6?`<div class="task-empty">+${taskItems.length-6} işlem daha var. Araçlar ekranından görebilirsin.</div>`:'');
}

      ...
      return `
        <div class="pkg-card ${isCurrent ? 'pkg-highlight' : ''}">
          ...
          ${isCurrent
            ...
            : kurumsalTier
              ? `<button class="change-mode-link" style="display:block; text-align:center;" onclick="openManageSubscriptions()">Pakete geçmek için Aboneliği Yönet'i kullanın</button>`
...
(function(){
  ...
  window.exportFullBackup=async function(){
    ...
    const payload={app:'Garage Book',version:'2.1',exportedAt:new Date().toISOString(),accountType,isProBireysel,kurumsalTier,vehicles,docs};
    const json=JSON.stringify(payload,null,2);
    const backupFileName='garage-book-tam-yedek-'+new Date().toISOString().slice(0,10)+'.json';
    if(hasNativeFilesystem()){
      const base64=btoa(unescape(encodeURIComponent(json)));
      await saveAndShareNative(base64,backupFileName);
    } else {
      const blob=new Blob([json],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=backupFileName;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);
    }
  };
  ...
  window.generateVehiclePdf=async function(id){
    ...
    const vehiclePdfName='garage-book-'+(v.plate||'arac').replace(/\s+/g,'-')+'.pdf';
    if(hasNativeFilesystem()){
      const dataUri=doc.output('datauristring');
      const base64=dataUri.slice(dataUri.indexOf(',')+1);
      await saveAndShareNative(base64,vehiclePdfName);
    } else {
      doc.save(vehiclePdfName);
    }
  };
